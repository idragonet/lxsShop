...
namespace lxsShop.NewServices.IBaseServices
{
    public class BaseService<T> : DbFactory, IBaseService<T> where T : class, new()
    {
        public async Task<ApiResult<string>> AddAsync(T parm, bool Async = true)
        {
            ...
            catch (Exception ex)
            {
                ...
                LogManager.Error( ApiEnum.Error.ToString(), ex.Message);
        ...
        /// <returns></returns>
        public async bool AddAsync(T parm, bool Async = true)
        {
            var res = new ApiResult<string>() { statusCode = (int)ApiEnum.Error };
            try
            {
